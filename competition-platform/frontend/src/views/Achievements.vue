  ...
  <div v-loading="loading">
    <el-card shadow="never" class="block">
      <template #header>
        <div class="card-head">
          <span>🏆 成果墙（获奖展示）</span>
          <el-button v-if="store.isLogin" type="primary" size="small" @click="openAdd">＋ 发布成果</el-button>
        </div>
      </template>
      <el-row :gutter="16">
        <el-col :span="8" v-for="a in list" :key="a.id" style="margin-bottom: 16px">
          <el-card shadow="hover">
            <div class="ach-head">
              ...
              <el-tag :type="levelType(a.level)" size="small">{{ a.level }}</el-tag>
            </div>
            ...
            <div class="ach-foot">
              <span class="ach-user">
                <span v-if="a.teamName && a.teamId" class="ach-link" @click="$router.push(`/teams/${a.teamId}`)">🏆 {{ a.teamName }}</span>
                <template v-if="a.teamName && a.userName"> · </template>
                <span class="ach-link" @click="$router.push(`/users/${a.userId}`)">👤 {{ a.userName }}</span>
    ...
    <el-card v-if="store.isLogin" shadow="never" class="block">
      ...
      <el-table :data="myList" size="small" v-loading="myLoading">
        ...
        <el-table-column label="状态" width="100">
          <template #default="{ row }">
            <el-tag :type="ACH_STATUS_TYPE[row.status] || 'info'" size="small">{{ row.status }}</el-tag>
          </template>
        </el-table-column>
      ...
      <el-empty v-if="!myList.length" description="还没有提交过成果" :image-size="60" />
    </el-card>

    <el-dialog v-model="addVisible" title="发布成果（需管理员审核）" width="480px">
      <el-alert type="warning" :closable="false" show-icon
        title="成果需上传证明文件（证书/奖状照片等），经管理员审核通过后才会公开展示" style="margin-bottom: 12px" />
      <el-form :model="form" label-width="80px">
        <el-form-item label="成果名称" required>
          <el-input v-model="form.name" placeholder="如：全国大学生数学建模竞赛省二等奖" />
        </el-form-item>
        <el-form-item label="所属竞赛">
          <el-select v-model="form.competitionId" clearable placeholder="选择竞赛" style="width: 100%">
            <el-option v-for="c in competitions" :key="c.id" :label="c.name" :value="c.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="获奖队伍">
          <el-select v-model="form.teamId" placeholder="选择获奖队伍或个人奖项" style="width: 100%">
            <el-option :value="null" label="🎖️ 个人奖项（不关联队伍）" />
            <el-option v-for="t in myTeams" :key="t.id" :label="`${t.title}（${t.competitionName}）`" :value="t.id" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="获奖情况" required>
          <el-input v-model="form.award" placeholder="如：省级二等奖 / 国赛铜奖" />
        </el-form-item>
        ...
        <el-form-item label="证明文件" required>
          ...
          >
            <el-button size="small" :loading="proofUploading">{{ form.proof ? '✅ 已上传，点击重新上传' : '＋ 上传证明（图片/PDF）' }}</el-button>
          </el-upload>
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="addVisible = false">取消</el-button>
        <el-button type="primary" :loading="saving" :disabled="!form.proof" @click="submit">提交审核</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
const ACH_STATUS_TYPE = { '待审核': 'warning', '已通过': 'success', '已拒绝': 'danger' }
...
const form = reactive({ competitionId: null, teamId: null, name: '', level: '校级', award: '', awardYear: new Date().getFullYear(), proof: '' })
...
async function load() {
  loading.value = true
  try {
    const data = await api.get('/achievements', { params: { page: page.value, size: pageSize } })
...
async function loadMy() {
  ...
  try {
    const data = await api.get('/achievements/my', { params: { page: 1, size: 50 } })
...
async function submit() {
  ...
  try {
    ...
    Object.assign(form, { competitionId: null, teamId: null, name: '', level: '校级', award: '', proof: '' })
    load()
    loadMy()
  } finally {
    saving.value = false
...
async function remove(a) {
  await ElMessageBox.confirm(`确定删除成果「${a.name}」吗？`, '删除确认', { type: 'warning' })
